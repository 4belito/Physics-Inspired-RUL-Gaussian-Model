...
def fill_info_plot_predRUL(plot_predRUL,RULs,EOLcauses,pred_names,t_observ,colors,n):
    ...
    if n==1:
        plot_predRUL['time'] = np.array([t_observ[n], t_observ[n]]) 
    else: 
        plot_predRUL['time'] = np.array(t_observ[n-2:n])
...
def make_rul_video(unit, t_np, t_observ, true_RUL, failure_cause, monot, threshold, group_networks, A, b, S, gamma, 
    ...
    setup_plot_predRUL(axes[ax_ids[0]], t_observ, true_RUL, failure_cause, causes_text, colors,time_unit=time_unit)
    for i,name in enumerate(perform_names):
        perform_pred_line[name]=setup_plot_perform(axes[ax_ids[i+1]],t_np,gpu2np(threshold[name]),t_observ, b[name], name, unit, y_lim[name], loc[name],time_unit)  
    ...
    frames = [make_frame(t) for t in t_observ.tolist()]
