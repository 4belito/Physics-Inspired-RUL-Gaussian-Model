...
def setup_plot_axes(unit,conserv,ood_coef):
    fig, axes = plt.subplots(2, 2, figsize=(10, 16))
    fig.suptitle(f'RUL Prediction with conserv={conserv} ood={ood_coef} (Testing UAV #{unit})', fontsize=16)
...
def setup_plot_axes(unit,conserv,ood_coef):
    ...
    #fig, axes = plt.subplots(1, 2, figsize=(12, 5)) #N-CMAPSS
    fig.suptitle(f'RUL Prediction with conserv={conserv} ood={ood_coef} (Testing UAV #{unit})', fontsize=16)
    plt.subplots_adjust(hspace=0.3, wspace=0.3)
    return fig, axes

def setup_plot_perform(ax,t,thres,time_sys, b, name, unit, y_lim, loc,time_unit='hours'): #time=t[:,0]
...
def make_rul_video(unit, t, time_sys, true_RUL, failure_cause, monot, threshold, group_networks, A, b, S, gamma, 
        y_lim, loc, time_unit,
    ...
    setup_plot_predRUL(axes[ax_ids[0]], time_sys, true_RUL, failure_cause, causes_text, colors,time_unit=time_unit)
    for i,name in enumerate(perform_names):
        perform_pred_line[name]=setup_plot_perform(axes[ax_ids[i+1]],t,gpu2np(threshold[name]),time_sys, b[name], name, unit, y_lim[name], loc[name],time_unit)  
...
def plot_perform_prediction(ax, time_sys, b, t, mean, std_Data, std_ModelData, std_ModelDataFut, thres, name, unit, y_lim, loc,time_unit, b_n, t_n, time_est=None, b_est=None):
    ax.set_xlabel(f'time ({time_unit})')
    ax.set_ylabel(f'{name}')
...
def make_perform_video(unit, group_network, thres, time_sys, A, b, S,
    ...
    alpha_init=np.ones(n_train_unit)/n_train_unit
    def make_frame(s):
        nonlocal alpha_init
        ax.clear()
        t_n = int(s * frames) + 1
        ...
        plot_perform_prediction(ax, time_sys, b, t, pred_perform, std_Data, std_ModelData, std_ModelDataFut, thres, name, unit, y_lim, loc,time_unit, bn, t_n, time_est, b_est)
