...
class PolyTaylorNet(nn.Module):
    def __init__(self, derivatives,a):
        super().__init__()
        self.derivatives = derivatives  # Reverse the coefficients
...
class MonotIsoLinear(nn.Module):
    def __init__(self, features, bias=True, positive_outs=None,monot_outs=None):
        ...
        self.register_buffer('positive_outs', positive_outs)
        self.register_buffer('monot_outs', monot_outs)
        #self.positive_outs = torch.tensor(positive_outs,device=device) if positive_outs else torch.tensor(out_features * [False],device=device)
...
class Holes(nn.Module):
    ...
    def get_weights_std(self):
        weights = [[param.clone().detach() for param in hole.parameters()] for hole in self.holes]
        weights_std = []
        for param_group in zip(*weights):
    ...
    def get_weights_zscores(self):
        ...
        weights_zscore = []
        for param_group in zip(*weights):
            param_group=torch.stack(param_group)
            mean_param = torch.mean(param_group, dim=0)
            std_param = torch.std(param_group, dim=0)
            weights_zscore.append((param_group-mean_param)/std_param)
    ...
    def get_sub_holes(self,holes_list):
        ...
        #sub_holes.load_net([hole for i,hole in enumerate(self.holes_in.holes) if i in holes_list])
...
class LatentF(nn.Module): # This is more or less a particular case of an AdaptID 
    def __init__(self, hidden_sizes=[3],channels_out=2,activation=nn.Sigmoid()):
        ...
        self.backbone.append(nn.Linear(1, hidden_sizes[0]))
        self.backbone.append(nn.LayerNorm(hidden_sizes[0]))
        self.skip.append(nn.Linear(1, hidden_sizes[0]))
        for i in range(1, len(hidden_sizes)):
            self.backbone.append(activation)
            self.backbone.append(nn.Linear(hidden_sizes[i-1], hidden_sizes[i]))
            self.backbone.append(nn.LayerNorm(hidden_sizes[i]))
            self.skip.append(nn.Linear(hidden_sizes[i-1], hidden_sizes[i]))
        # Create the output layer
        self.backbone.append(activation)
        self.linear_output=nn.Linear(hidden_sizes[-1],channels_out)    
        self.skip.append(nn.Linear(hidden_sizes[-1],channels_out))   
    ...
    def forward(self, x):
        for i,layer in enumerate(self.backbone):
            if i%3==0:
                x_skip=x
            if i%3==1:
                x = layer(x)+self.skip[i//3](x_skip)
            else:
                x = layer(x)
        out=self.linear_output(x)+self.skip[-1](x_skip)
        return out
...
class WaterCan(nn.Module):
    ...
    def get_holes_std(self):
        return torch.cat(self.holes_in.get_weights_std()+self.holes_out.get_weights_std())

    def get_holes_zscores(self):
        return torch.cat(self.holes_in.get_weights_zscores()+self.holes_out.get_weights_zscores(),dim=1) 

    def get_holes_zscores_norm(self):
        return torch.norm(self.get_holes_zscores(),p=2,dim=1)
...
class GroupWaterCan(nn.Module):
    def __init__(self,configs):
        ...
        assert all(configs[i]['n_holes'] == n_holes for i in range(n_models)), 'All watercans must have the same number of holes'
    ...
    def forward_lc(self,x, alpha,out_dist=True,out_std=True): #alpha_tensor = torch.tensor(alpha, dtype=torch.float32)
